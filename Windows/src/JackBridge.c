...
static RuleAction match_rule(const char *process_name, UINT32 dest_ip, UINT16 dest_port, BOOL is_udp)
{
    ...
    while (rule != NULL)
    {
        ...
        if ((strcmp(rule->process_name, "ANY") == 0 || match_process_list(rule->process_name, process_name)) &&
...
JACKBRIDGE_API UINT32 JackBridge_GetRulePosition(UINT32 rule_id)
{
    if (rule_id == 0)
        return 0;
...
JACKBRIDGE_API BOOL JackBridge_MoveRuleToPosition(UINT32 rule_id, UINT32 new_position)
{
...
JACKBRIDGE_API BOOL JackBridge_SetProxyConfig(ProxyType type, const char* proxy_ip, UINT16 proxy_port, const char* username, const char* password)
{
...
JACKBRIDGE_API void JackBridge_ClearConnectionLogs(void)
{
    clear_logged_connections();
