...
static RuleAction match_rule(const char *process_name, uint32_t dest_ip, uint16_t dest_port, bool is_udp)
{
    ...
    while (rule != NULL)
    {
        ...
        // Rules are evaluated strictly in priority order. Put catch-all rules last
        // when they should behave as a fallback.
        if ((strcasecmp(rule->process_name, "ANY") == 0 || match_process_list(rule->process_name, process_name)) &&
...
uint32_t JackBridge_AddRule(const char* process_name, const char* target_hosts, const char* target_ports, RuleProtocol protocol, RuleAction action)
{
...
bool JackBridge_EnableRule(uint32_t rule_id)
{
    if (rule_id == 0)
        return false;
...
bool JackBridge_DisableRule(uint32_t rule_id)
{
    if (rule_id == 0)
        return false;
...
bool JackBridge_DeleteRule(uint32_t rule_id)
{
    if (rule_id == 0)
        return false;
...
bool JackBridge_EditRule(uint32_t rule_id, const char* process_name, const char* target_hosts, const char* target_ports, RuleProtocol protocol, RuleAction action)
{
...
bool JackBridge_SetProxyConfig(ProxyType type, const char* proxy_ip, uint16_t proxy_port, const char* username, const char* password)
{
...
void JackBridge_SetDnsViaProxy(bool enable)
{
    g_dns_via_proxy = enable;
    log_message("dns via proxy %s", enable ? "enabled" : "disabled");
}

void JackBridge_SetLogCallback(LogCallback callback)
{
    g_log_callback = callback;
}

void JackBridge_SetConnectionCallback(ConnectionCallback callback)
{
    g_connection_callback = callback;
}

void JackBridge_SetTrafficLoggingEnabled(bool enable)
{
    g_traffic_logging_enabled = enable;
}

void JackBridge_ClearConnectionLogs(void)
{
    clear_logged_connections();
}

bool JackBridge_Start(void)
{
    if (running)
        return false;
...
int JackBridge_TestConnection(const char* target_host, uint16_t target_port, char* result_buffer, size_t buffer_size)
{
    int test_sock = -1;
    struct sockaddr_in proxy_addr;
    uint32_t target_ip;
